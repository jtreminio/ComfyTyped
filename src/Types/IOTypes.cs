...
/// <summary>Maps ComfyUI type name strings to their C# marker types at runtime. Extensions can call <see cref="Register"/> to add custom marker types.</summary>
public static class IOTypeMap
{
    private static readonly Dictionary<string, Type> Map = new(StringComparer.OrdinalIgnoreCase)
    {
        ["MODEL"] = typeof(ModelType),
        ["CLIP"] = typeof(ClipType),
        ["VAE"] = typeof(VaeType),
        ["LATENT"] = typeof(LatentType),
        ["IMAGE"] = typeof(ImageType),
        ["MASK"] = typeof(MaskType),
        ["CONDITIONING"] = typeof(ConditioningType),
        ["AUDIO"] = typeof(AudioType),
        ["VIDEO"] = typeof(VideoType),
        ["INT"] = typeof(IntType),
        ["FLOAT"] = typeof(FloatType),
        ["STRING"] = typeof(StringType),
        ["BOOLEAN"] = typeof(BooleanType),
        ["SAMPLER"] = typeof(SamplerType),
        ["SIGMAS"] = typeof(SigmasType),
        ["GUIDER"] = typeof(GuiderType),
        ["NOISE"] = typeof(NoiseType),
        ["CLIP_VISION"] = typeof(ClipVisionType),
        ["CLIP_VISION_OUTPUT"] = typeof(ClipVisionOutputType),
        ["STYLE_MODEL"] = typeof(StyleModelType),
        ["CONTROL_NET"] = typeof(ControlNetType),
        ["GLIGEN"] = typeof(GligenType),
        ["HOOKS"] = typeof(HooksType),
        ["UPSCALE_MODEL"] = typeof(UpscaleModelType),
        ["LATENT_UPSCALE_MODEL"] = typeof(LatentUpscaleModelType),
        ["IPADAPTER"] = typeof(IpAdapterType),
        ["MODEL_PATCH"] = typeof(ModelPatchType),
        ["LORA_MODEL"] = typeof(LoraModelType),
        ["BBOX"] = typeof(BboxType),
        ["TORCH_COMPILE_ARGS"] = typeof(TorchCompileArgsType),
        ["*"] = typeof(AnyType),
        ["COMFY_MATCHTYPE_V3"] = typeof(ComfyMatchTypeV3),
    };

    /// <summary>Resolve a ComfyUI type name to its C# marker type. Returns typeof(AnyType) for unknown types.</summary>
    public static Type Resolve(string typeName) =>
        Map.TryGetValue(typeName, out Type? t) ? t : typeof(AnyType);

    /// <summary>Check if a type name is known.</summary>
    public static bool IsKnown(string typeName) => Map.ContainsKey(typeName);

    /// <summary>All ComfyUI type name strings currently registered.</summary>
    public static IEnumerable<string> KnownTypeNames => Map.Keys;

    /// <summary>
    /// Register a custom <see cref="IComfyType"/> marker for a ComfyUI type name.
    /// Extensions call this at startup to wire their custom types into the map.
    /// Re-registering an existing name overwrites the prior mapping.
    /// </summary>
    public static void Register<T>(string typeName) where T : IComfyType =>
        Map[typeName] = typeof(T);

    /// <summary>Register a custom marker type by reflection. Throws if the type does not implement <see cref="IComfyType"/>.</summary>
    public static void Register(string typeName, Type markerType)
    {
        if (!typeof(IComfyType).IsAssignableFrom(markerType))
        {
            throw new ArgumentException(
                $"Type {markerType.FullName} does not implement IComfyType.", nameof(markerType));
        }
        Map[typeName] = markerType;
    }
}
