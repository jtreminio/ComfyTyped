...
- **`src/Generated/`** — ~700 `*.g.cs` files emitted by `tools/ComfyTyped.CodeGen` from `object_info.json`. Each file declares one `ComfyNode` subclass plus its inputs/outputs typed with `IComfyType` markers, and a fluent `With(...)` method exposing primitive inputs (INT/FLOAT/STRING/BOOL) as nullable named parameters — `new KSamplerNode().With(Seed: 42, Steps: 20)` returns `this` for chaining. Connection inputs are intentionally not in `With(...)`; they stay on `.ConnectTo(...)` / `.ConnectToUntyped(...)` so type-mismatch stays a compile error. `NodeRegistrations.g.cs` is the codegen's registration entry point — call `ComfyTyped.Generated.NodeRegistrations.EnsureRegistered()` once at process startup. **Never hand-edit these files**; regenerate.
