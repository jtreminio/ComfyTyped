...
- **`src/Generated/`** — ~700 `*.g.cs` files emitted by `tools/ComfyTyped.CodeGen` from `object_info.json`. Each file declares one `ComfyNode` subclass plus its inputs/outputs typed with `IComfyType` markers, and a fluent `With(...)` method exposing **every singular input** as a nullable named parameter — `new KSamplerNode().With(Seed: 42, Steps: 20, Model: ckpt.MODEL, LatentImage: empty.LATENT)` returns `this` for chaining. Each parameter takes an *input binding* (`src/Core/InputBindings.cs`): primitive inputs (INT/FLOAT/STRING/BOOL) accept a literal **or** a same-typed output via `IntArg`/`FloatArg`/`StringArg`/`BoolArg`, and connection inputs accept a same-typed output via `In<T>`. Type-mismatch stays a compile error — there is no implicit conversion from a wrong-typed `NodeOutput<T>` to the binding, so `With(Model: ckpt.LATENT)` won't compile. You never name the binding types yourself (`int → IntArg → IntArg?` lifts automatically at the call site). `.ConnectTo(...)` / `.ConnectToUntyped(...)` / `.Set(...)` remain the low-level primitives `With(...)` builds on (and what the bridge/deserialization use directly). **Input lists (`COMFY_AUTOGROW_V3`) are not in `With(...)`** — use `Add`/`AddRange`. Combo (enum) string inputs additionally get a nested `public static class {Input}Values` of `public const string` for their known ComfyUI values (`ImageScaleNode.UpscaleMethodValues.Lanczos == "lanczos"`) — discoverable suggestions only; the input stays a plain string so any value still compiles (the C# analogue of TS `"a" | (string & {})`). Large/dynamic pickers (model/LoRA/file lists, or >64 values) are skipped. `NodeRegistrations.g.cs` is the codegen's registration entry point — call `ComfyTyped.Generated.NodeRegistrations.EnsureRegistered()` once at process startup. **Never hand-edit these files**; regenerate.

- **`src/SwarmUI/`** — the SwarmUI integration layer. Lives in namespace `ComfyTyped.SwarmUI` (deliberately separate from `ComfyTyped.Core` to keep SwarmUI-coupled types visually distinct). `MediaRef` is the typed equivalent of SwarmUI's `WGNodeData` (typed `INodeOutput` plus media metadata: dimensions, FPS, `T2IModelCompatClass`); converts to/from `WGNodeData` at the boundary, and `input.ConnectFrom(mediaRef)` (in `MediaRefExtensions`) connects a slot straight from one. **Bridge factory: `using WorkflowBridge bridge = BridgeSync.For(g);`** — returns a plain `WorkflowBridge` (no wrapper type) wired with an `INodeIdCounter` adapter over `WorkflowGenerator.LastID`, so every `AddNode`/`AddStub` advances `g.LastID` past the new node at add time (and the ctor sweeps any pre-existing IDs once), keeping SwarmUI's counter in lockstep so a later `g.CreateNode()` never collides. The bridge also auto-syncs typed mutations on subscribed nodes into the JObject, so the `WorkflowBridge.Create` + per-node `SyncNode` + trailing `SyncLastId` ritual collapses to one `using` — an explicit `SyncNode` is needed only after `ExtraInputs`/`RawInputs` edits (which emit no change event). Because the counter lives on the bridge **instance**, passing it through a `(WorkflowBridge bridge, …)` helper keeps the same live counter, so a *delegated* `bridge.AddNode(...)` advances `g.LastID` just the same — there is **no wrapper-unwrap caveat** (the former `SyncingWorkflowBridge` is deleted). Reach for the lower-level **`WorkflowBridge.Create(g.Workflow)`** (no counter) only for **read-only bridges** (resolve/query with no `AddNode` — avoids a needless `g.LastID` write) or the rare multi-phase adder that re-creates the bridge mid-method and hand-manages IDs with `BridgeSync.SyncLastId(g)` (currently only `LtxAudioInjector`). **`INodeIdCounter` (`src/Core/INodeIdCounter.cs`) is Core's only host-ID coupling** — Core knows "something with a `LastID`", never `WorkflowGenerator`; `BridgeSync.For` supplies the adapter and core SwarmUI is never modified. Construct a bridge with no counter (Core tests, deserialization, host-agnostic uses) and it does no ID bookkeeping.
